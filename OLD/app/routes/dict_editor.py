...
router = APIRouter()
templates = Jinja2Templates(directory="OLD/app/templates")
...
DICT_FILES = {
    "一般用語": {
        "file": "OLD/ocr/dic/known_words_common.csv",
        "description": "契約書、請求書などの一般的な業務用語",
        "type": "single_column",
        "header": "common_word"
    },
    "専門用語": {
        "file": "OLD/ocr/dic/known_words_custom.csv", 
        "description": "航宇、熱交などの専門用語・略語",
        "type": "single_column",
        "header": "custom_word"
    },
    "誤字修正": {
        "file": "OLD/ocr/dic/ocr_word_mistakes.csv",
        "description": "OCRでよく発生する誤字とその修正",
        "type": "two_column",
        "headers": ["wrong", "correct"]
    },
    "形態素辞書": {
        "file": "OLD/ocr/dic/user_dict.csv",
...
async def save_dict_content(dict_name: str, content: str = Form(...)):
    ...
    try:
        # バックアップを作成
        if os.path.exists(file_path):
            # バックアップファイルはOLD/ocr/dic/back/に保存
            backup_dir = "OLD/ocr/dic/back"
