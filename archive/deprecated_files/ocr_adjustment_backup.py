...
class OCRAdjustmentPage:
    ...
    def _open_dict_editor(self, label: str, filename: str):
        ...
        with ui.dialog() as dialog:
            with ui.card().style('width: 85vw; height: 80vh; margin: 0; display: flex; flex-direction: column; overflow: hidden;'):
                # ヘッダー
                ...
                # メインエディタエリア
                with ui.element('div').style('flex: 1; min-height: 0; padding: 16px; display: flex; flex-direction: column; gap: 12px;'):
                    ui.label('📝 内容を編集してください（1行に1項目）').style('font-size: 13px; color: #6b7280;')
                    textarea = ui.textarea(value=content).style(
                        'width: 100%; height: 100%; min-height: 400px; font-family: monospace; font-size: 13px; '
                        'resize: none; box-sizing: border-box; white-space: nowrap; overflow-wrap: normal;'
                    ).props('outlined')
                ...
                with ui.element('div').style('display: flex; justify-content: flex-end; gap: 8px; padding: 12px 16px; border-top: 1px solid #e5e7eb; background: #f9fafb;'):
                    ui.button('💾 保存', on_click=lambda d=dialog, t=textarea: save_and_close(d, t)).props('unelevated')
                    ui.button('キャンセル', on_click=dialog.close).props('flat')
        dialog.open()
