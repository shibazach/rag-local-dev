...
# REM: rag-app など Docker 側は host.docker.internal:11434 で接続
...
# REM: rag_ragnet ネットワークのサブネットを自動取得
...
if [[ -n "$DOCKER_NET" ]]; then
  ...
  echo "⚠️  rag_ragnet network not found; skipping firewall rules."
fi
...
echo -e "\n🎉 Setup complete!  rag-app からは http://host.docker.internal:11434 で到達できます."
  z