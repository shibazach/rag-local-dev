...
class RAGHeader:
    ...
    def create_header(self):
        ...
        with ui.element('div').style('position:fixed;top:0;left:0;right:0;width:100%;height:48px;background:#334155;z-index:1000;display:flex;align-items:center;padding:0;margin:0;overflow:hidden;'):
            ...
            # 中央：ナビゲーション（レスポンシブ・全ページ表示・現在ページ赤字）
            with ui.element('div').style('flex:1;display:flex;align-items:center;justify-content:center;gap:8px;flex-wrap:wrap;min-height:48px;padding:0 8px;'):
    ...
    def _nav_button(self, icon: str, label: str, path: str, is_current: bool = False):
        ...
        with ui.element('div').style(f'display:flex;align-items:center;gap:4px;cursor:{cursor_style};padding:4px 8px;border-radius:4px;transition:background 0.2s;white-space:nowrap;').on('click', click_handler):
            ui.label(icon).style(f'color:{text_color};font-size:16px;')
            ui.label(label).style(f'color:{text_color};font-size:14px;')
